...
namespace ARKGame
{
    public class ProcedureCheckVersion : ProcedureBase
    {
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            ARKGameEntry.Event.Subscribe(WebRequestSuccessEventArgs.EventId, OnWebRequestSuccess);
            ARKGameEntry.Event.Subscribe(WebRequestFailureEventArgs.EventId, OnWebRequestFailure);
            ARKGameEntry.Event.Subscribe(ResourceInitCompleteEventArgs.EventId, OnResourceInitComplete);
        ...
        {
            ARKGameEntry.Event.Unsubscribe(WebRequestSuccessEventArgs.EventId, OnWebRequestSuccess);
            ARKGameEntry.Event.Unsubscribe(WebRequestFailureEventArgs.EventId, OnWebRequestFailure);
            ARKGameEntry.Event.Unsubscribe(ResourceInitCompleteEventArgs.EventId, OnResourceInitComplete);
        ...
        private void RequestVersion()
        {
...
#endif
            string gameVersion = ARKGameEntry.Base.GameVersion;
            string platform = Application.platform.ToString();
            string language = ARKGameEntry.Localization.Language.ToString();
            ...
            ARKGameEntry.WebRequest.AddWebRequest(ARKGameEntry.Config.BuildInfo.CheckVersionUrl, wwwForm, this);
        }
        ...
        private void OnWebRequestSuccess(object sender, GameEventArgs e)
        {
            ...
            Log.Info("Latest game version is '{0}', local game version is '{1}'.", versionInfo.LatestGameVersion, ARKGameEntry.Base.GameVersion);

            if (versionInfo.ForceGameUpdate)
            {
                ARKGameEntry.UI.OpenDialog(new DialogParams
                { 
                    Mode = 2,
                    Title = ARKGameEntry.Localization.GetString("ForceUpdate.Title"),
                    Message = ARKGameEntry.Localization.GetString("ForceUpdate.Message"),
                    ConfirmText = ARKGameEntry.Localization.GetString("ForceUpdate.UpdateButton"),
                    OnClickConfirm = delegate (object userData) { Application.OpenURL(versionInfo.GameUpdateUrl); },
                    CancelText = ARKGameEntry.Localization.GetString("ForceUpdate.QuitButton"),
            ...
            ARKGameEntry.Resource.InitResources();
        }
