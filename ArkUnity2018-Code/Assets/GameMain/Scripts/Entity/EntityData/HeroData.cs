...
namespace ARKGame
{
    ...
    public class HeroData : TargetableObjectData
    {
        ...
        {
            IDataTable<DRHero> dtHero = ARKGameEntry.DataTable.GetDataTable<DRHero>();
            DRHero drHero = dtHero.GetDataRow(TypeId);
            if(drHero == null)
            {
                return;
            }
            m_ThrusterData = new ThrusterData(ARKGameEntry.Entity.GenerateSerialId(), drHero.ThrusterId, Id, Camp);
            for(int index = 0, weaponId = 0; (weaponId = drHero.GetWeaponIds(index))>0; index++)
            {
                AttachWeaponData(new WeaponData(ARKGameEntry.Entity.GenerateSerialId(), weaponId, Id, Camp));
            }
            for(int index = 0, armorId = 0; (armorId = drHero.GetArmorIds(index)) > 0; index++)
            {
                AttachArmorData(new ArmorData(ARKGameEntry.Entity.GenerateSerialId(), armorId, Id, Camp));
            }
            m_DeadEffectId = drHero.DeadEffectId;
            m_DeadSoundId = drHero.DeadSoundId;
            HP = m_MaxHP;
        }
